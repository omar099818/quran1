...
client.on('ready', function(){
    var ms = 30000;
    var setGame = [`-help`,'GamingArmy','By: Omar_9897'];
    var i = -1;
    var j = 0;
    setInterval(function (){
        ...
        i = i+j;
        client.user.setGame(setGame[i],`http://www.twitch.tv/omar_9897`);
    }, ms);
 ...
 client.on('message', message => {
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .setFooter('All CopyRights Reseaved For GamingArmy team')
    })
}
});
...
client.on('message', message => {
	...
	.setColor('#000000')
	.setFooter("G.a Quran", 'https://images-ext-2.discordapp.net/external/M2_qC9yi0p7fk7a7c62C5hRlltTO5GJgwo7le8Sg2co/%3Fsize%3D2048/https/cdn.discordapp.com/avatars/487049939617316864/48de62e1296ef2feaacd50a6509ae19f.png')
      .setDescription(` 
...
collector1.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=Ktync4j_nmA", { filter: 'audioonly' });
...
collector2.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=qFq5h4wtjaM&t=30s", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector3.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=8UWKiKGQmsE", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector5.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=vqXLGtZcUm8", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector6.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=WYT0pQne-7w", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
...
collector7.on('collect', r => {
    voiceChannel.join()
      .then(connnection => {
        const stream = yt("https://www.youtube.com/watch?v=LTRcg-gR78o", { filter: 'audioonly' });
        const dispatcher = connnection.playStream(stream);
        dispatcher.on('end', () => voiceChannel.leave());
		collector1.stop();
		collector2.stop();
